...
def safe_text_field(item: JsonMap, key: str, default: str) -> str:
    ...
    redacted = redact(raw)
    return "<redacted-metadata>" if redacted != raw else redacted
...
def lines_field(item: JsonMap) -> str:
    ...
    if isinstance(affected, list) and affected:
        ...
        for value in affected:
            ...
            line_text = ", ".join(str(line) for line in lines) if isinstance(lines, list) else "unknown"
...
def standards_field(item: JsonMap) -> str:
    ...
    if isinstance(refs, list) and refs:
        values = [redact(value) for value in refs if isinstance(value, str)]
        if values:
            return ", ".join(values)
    ...
    return {
        "authz": "OWASP ASVS V4",
        "authn": "OWASP ASVS V2",
        "injection": "OWASP ASVS V5",
        "ssrf": "OWASP ASVS V12",
        "secrets": "OWASP ASVS V6",
        "supply-chain": "OWASP SCVS",
        "ci-cd": "OWASP SCVS",
        "ai-agentic": "OWASP LLM Top 10",
    }.get(category, "Not mapped")


def category_summary(sections: ReportSections) -> str:
    categories = sorted({safe_text_field(item, "category", "unknown") for item in [*sections.confirmed, *sections.resolved, *sections.candidates]})
...
def render_items(lines: list[str], group: RenderGroup) -> int:
    ...
    for item in group.items:
        title = safe_text_field(item, "title", text_field(item, "matcher_id", "Needs review"))
        lines.extend([
            ...
            "**Attack scenario**",
            "",
            redact(text_field(item, "attack_scenario", "Not confirmed. Validate reachability and attacker control before claiming an attack path.")),
            "",
            "**Impact**",
            "",
            redact(text_field(item, "impact", text_field(item, "reason", "Candidate needs validation before impact is claimed."))),
            "",
            "**Recommendation**",
            "",
            redact(text_field(item, "recommendation", text_field(item, "review_prompt", "Validate the code path, security boundary, and exploitability before confirming."))),
            "",
            "**Verification**",
            "",
            redact(text_field(item, "verification", "Trace reachability and add a regression test before marking this confirmed or fixed.")),
            "",
        ])
        index += 1
    return index
...
def report_payload(input_path: Path, output_path: Path | None) -> JsonMap:
    ...
    report = render_report(report_sections_from_json(data))
